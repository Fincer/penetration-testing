...
ja koska normaalikäyttäjällä `PATH` -ympäristömuuttujaan ei ole sisällytetty arvoa `/usr/sbin`. **HUOM:** Seuraava toimenpide ei ole suositeltava, vaan on suositeltavampaa käyttää oletushakemistoa `/usr/local/bin/` yksittäisille skripteille ja binääreille.

Voimme kuitenkin muuttaa järjestelmän oletusasetusta `PATH`-muuttujalle suorittamalla `root`-tunnuksella seuraavan komentosarjan:
...
Se, kumpi hyökkäystyyli voitaisiin näiden kahden välillä valita, riippuu keskeisesti siitä, minkälaista käyttäjän syötettä odotetaan. Onko syöte `string`, `integer`, `boolean`, `float`. 
...
Reverse TCP -yhteystavassa _palvelin ottaa yhteyden asiakasohjelmaan_ (ja odottaa sitä), mikä on päin vastoin kuin normaalissa asiakas/palvelin-yhteydessä (Forward connection). Normaalisti _asiakas ottaa yhteyden palvelimeen_.
...
minkä jälkeen suoritetaan Windows-koneella ladattu `kali.exe`, joka ottaa yhteyden kuuntelevaan Metasploit-prosessiin Kali Linuxissa.
